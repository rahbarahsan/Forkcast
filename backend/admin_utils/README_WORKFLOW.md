...
2. **Configure your settings**
   - Copy `admin_utils/config/pipeline_config_example.json` to `admin_utils/config/pipeline_config.json`
...
```bash
cd backend
python -m admin_utils.scripts.enrich_ingredients_from_supabase
```
...
- Creates CSV file in `admin_utils/csv/` folder
- Also creates copy in `admin_utils/csv/finalized_csv/` folder
...
1. **Review the generated CSV** in `admin_utils/csv/finalized_csv/`
...
```bash
cd backend
python -m admin_utils.scripts.finalize_corrections_local
```
...
```bash
cd backend
python -m admin_utils.scripts.push_to_grocery_items
```
...
```bash
cd backend
python -m admin_utils.scripts.validate_lookup_table
```
...
```bash
cd backend
python -m admin_utils.scripts.resolve_lookup_conflicts
```
...
### Pipeline Config (`admin_utils/config/pipeline_config.json`)
...
- `admin_utils/config/.env` - Supabase and OpenAI credentials
